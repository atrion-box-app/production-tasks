...
# --- Google Sheets API Connection via Base64 ---
@st.cache_resource
...
def select_project_card(p_name):
    st.session_state["selected_project_tab"] = p_name

tab_dash, tab_proj_cards, tab_proj, col_plan, tab_tech, tab_projec, tab_rep, tab_data = st.tabs([
    "📈 Dashboard",
    "📇 Project Cards", 
    "📋 Καρτέλα Project", 
    "🗓️ Ημερήσιο Πλάνο", 
...
# --- 2. PROJECT CARDS (GRID VIEW) ---
with tab_proj_cards:
    st.header("📇 Project Cards & Επισκόπηση")
    
    if not procurement_df.empty:
        projects_list = sorted([p for p in procurement_df["Project"].unique().tolist() if p != "-"])
        
        col_s, col_f = st.columns([2, 1])
        search_card = col_s.text_input("🔍 Αναζήτηση Project:", placeholder="Πληκτρολογήστε όνομα project...", key="card_search")
        filter_status = col_f.selectbox("📦 Φίλτρο Κατάστασης:", ["Όλα", "Σε Εκκρεμότητα", "Ολοκληρωμένα"])
        
        if search_card:
            projects_list = [p for p in projects_list if search_card.lower() in p.lower()]
            
        st.subheader(f"📊 Σύνολο Projects: {len(projects_list)}")
        st.divider()

        # Πλέγμα 3 Στηλών για τις Κάρτες
        cols = st.columns(3)
        for idx, p_name in enumerate(projects_list):
            ...
            for p_idx, r in filtered_p.iterrows():
                item_id = str(r["ID"])
                u_key = f"{item_id}_{p_idx}"
                qty = int(r["Ποσότητα"]) if str(r["Ποσότητα"]).isdigit() else 1
            ...
            if isinstance(p_tasks_dict, dict):
                for t_name, p_data in p_tasks_dict.items():
                    if isinstance(p_data, dict) and p_data.get("active", False):
                        auto_t = tasks_database.get(t_name, 0.0)
                        p_total_hrs += (auto_t * p_main_qty) / 60
                        p_tasks_cnt += 1
                        if p_data.get("done", False):
                            p_done_cnt += 1
            ...
            with col:
                with st.container(border=True):
                    ...
                    st.caption(f"• **Υλικά:** {len(filtered_p)} τμχ | • **Tasks:** {p_done_cnt}/{p_tasks_cnt}")
                    st.write(f"⏱️ **{round(p_total_hrs, 1)} Ώρες** | 📊 **{p_progress}%**")
                    st.progress(p_progress / 100)
                    
                    if st.button("👁️ Δες Λεπτομέρειες", key=f"btn_card_{p_name}", use_container_width=True):
                        st.session_state["selected_project_tab"] = p_name
                        st.rerun()
...
with tab_proj:
    ...
    if not procurement_df.empty:
        ...
        default_index = 0
        if "selected_project_tab" in st.session_state and st.session_state["selected_project_tab"] in projects_list:
            default_index = projects_list.index(st.session_state["selected_project_tab"])

        selected_project = st.selectbox("Επιλέξτε Project:", projects_list, index=default_index)
        ...
        if st.button("💾 Αποθήκευση Αλλαγών στο Google Sheet", key=f"save_btn_{selected_project}", use_container_width=True):
            save_all_assignments_to_sheet()
            st.success("✅ Όλες οι αναθέσεις αποθηκεύτηκαν επιτυχώς στο Google Sheet!")

# --- 4. ΗΜΕΡΗΣΙΟ ΠΛΑΝΟ (INTERACTIVE ΜΕ ALERTS & EXPORT) ---
with col_plan:
...
# --- 5. ΠΡΟΓΡΑΜΜΑ ΤΕΧΝΙΤΗ (INTERACTIVE ΜΕ ALERTS & EXPORT) ---
with tab_tech:
